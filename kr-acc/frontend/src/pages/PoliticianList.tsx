...
import { usePoliticians, useParties, useRegionSeats } from "@/api/queries";
...
export default function PoliticianList() {
  ...
  const regionSeats = useRegionSeats(termFilter ?? 22);
  ...
  // Filter out parties with 0 members from the politician data
  const activeParties = parties.data?.filter((p) => {
    // If we have data, check if any politician belongs to this party
    return true; // We'll filter by count below
  });
      ...
      <div className="mt-4 grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1">
          <div className="rounded-lg border border-gray-200 bg-white p-4">
            ...
            {regionSeats.data && regionSeats.data.length > 0 && (
              <KoreaMap data={regionSeats.data} />
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          ...
          <div className="space-y-3">
            ...
            {/* Party filter — exclude 0-count parties */}
            <div className="flex flex-wrap gap-1">
              <button
              ...
              {parties.data
                ?.filter((p) => {
                  // Only show parties that have members in the current view
                  // We approximate by checking the region data
                  if (!regionSeats.data) return true;
                  return regionSeats.data.some(
                    (r) => r.parties[p.name] && r.parties[p.name] > 0,
                  );
                })
                .map((p) => (
                  <button
